...
class LogicExpression
  def initialize
    @exprParser = Parser.new("Logic Expressions") do
      token(/\s+/)
      token(/\w+/) {|m| m}
      token(/./) {|m| m }
      ...
      rule :term do
        match('false') {|_| false}
        match('true') {|_| true}
        #match(:var) måste vara längst ner för att inte få tillbaka "true" eller "false" som strängar.
        match(:var)
      end
  ...
  def log(state = true)
    if state
      @exprParser.logger.level = Logger::DEBUG
    else
      @exprParser.logger.level = Logger::WARN
    end
  end
end
