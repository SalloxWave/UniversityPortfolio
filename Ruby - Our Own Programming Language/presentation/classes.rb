...
class Scope
  ...
  def Scope.set(new_level)
    ...
    #Decrease scope
    if new_level < @@level
      #Logically "-1" should be removed but that doesn't work now
      count = @@level - new_level
      count.times do 
        Scope.decrease
      end    
    #Increase scope
...
class Assignment
  ...
  def evaluate
    ...
    if @var.is_a?(ArrayCall)
      eval("old_var[Scope.get_level_by_name(@var.name)][@var.name][@var.index]#{@assign_op}@expression.evaluate")
    else
