...
class BinaryLanguage  
  def initialize
    @binaryParser = Parser.new("binary language") do

      variables = Hash.new

      token(/\s+/)
      token(/[a-zA-Z][a-zA-Z\d]*/) {|m| m }      
      token(/\d+/)
      token(/\w+/) {|m| m } 
      token(/./) {|m| m }
      ...
      rule :ASSIGN do
        match('(', 'set', :VAR, :EXPR, ')') {|_, _, v, e, _ | variables[v] = e }
      end

      rule :EXPR do
        match('(', 'or', :EXPR, :EXPR, ')') {|_, _, e, e2, _| (e or e2) }
        match('(', 'and', :EXPR, :EXPR, ')') {|_, _, e, e2, _| (e and e2) }
        match('(', 'not', :EXPR, ')') {|_, _, e, _| ( !e ) }
        match(:TERM)
      end
  ...
  def start
    print "[binarylanguage] "
    str = gets
    if done(str) then
      puts "Bye."
    else
      puts "=> #{@binaryParser.parse str}"
      start
    end
  end
  ...
  def log(state = true)
    if state
      @binaryParser.logger.level = Logger::DEBUG
    else
      @binaryParser.logger.level = Logger::WARN
    end
  end
end

BinaryLanguage.new.start
