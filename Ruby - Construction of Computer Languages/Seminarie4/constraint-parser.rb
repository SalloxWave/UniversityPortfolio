...
class ConstraintParser < Parser
  ...
  def initialize
    ...
    @parser=super("constraints") do
      ...
      #################################
      ###     THIS IS A PROBLEM!    ###
      ### Forcing the thing on the right of + or - to be a term 
      ### greatly limits the number of equations you can parse.
      ### get_connectors on something that is not necessarily a
      ### a connector is a bad idea? 
      
      ### f-32 is seen as a whole connector? "f-32 got new value: -32"
      ### should be split further and 32 should be a ConstantConnector.
      ...
      rule :expr do
        ...
        match(:expr, '-', :term) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'-',b)
          # a-b=c <=> b+c=a
          Adder.new(conn_c,conn_b,conn_a)
        end
  ...
  def get_connectors(conn_a,op,conn_b)
    ...
    ## Checks if either conn_a or conn_b isn't a connector.
    ## if the connector has a - in it get out not '.out' but '.a'.

    if ( !conn_a.is_a?(Connector) )
      if ( conn_a.a.to_s.include?('-') )
        conn_a = conn_a.a
      else
        conn_a = conn_a.out
      end
...
# puts
# f.user_assign 0
# puts "   f: #{f.value} c: #{c.value} (correct: -18)"
# f.user_assign 100
# puts "   f: #{f.value} c: #{c.value} (correct: 37)"
# f.forget_value 'user'
# c.user_assign 0
# puts "   c: #{c.value} f: #{f.value} (correct: 32)"
# c.user_assign 100
# puts "   c: #{c.value} f: #{f.value} (correct: 212)"
