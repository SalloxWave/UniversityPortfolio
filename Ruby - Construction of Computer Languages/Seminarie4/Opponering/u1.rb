...
# Vi ändrade i funktionen new_value
# Lade till två elsif för att hantera när out eller b ändras likväl som a gör i ursprungliga if-satsen.
# Vi behövde också kalla på inverse_op istället för op för att subtrahera/dividera istället.
...
def c2f
  ...
  cel = Connector.new("cel") # Celsius
  fah = Connector.new("fah") # Fahrenheit
  con1 = ConstantConnector.new("con1",32) # 32
  con2 = ConstantConnector.new("con2",1.8) # 1.8
  tmp = Connector.new("tmp") # temp

  Adder.new(tmp, con1, fah)
  Multiplier.new(cel, con2, tmp)

  #con1.user_assign(32)
  #con2.user_assign(1.8)
 # cel.user_assign(10)
