...
class ConstraintParser < Parser
  ...
  def get_connectors(conn_a,op,conn_b)
    ...
    if ( !conn_a.is_a?(Connector) )                 ## if either connector is a fraud.
      conn_a = conn_a.out                           ## turn that non-connector into a connector.
    end
  ...
  #Denna uppgift anfaller oss så vi måste försvara oss.
  #Slåss! Slåss! Aj! Aj! Åh nej uppgiften ligger före, vad ska vi ta oss till?
  #Aha, kaffe till undersättning, det heroiska kaffet hjälper oss besegra uppgiften.
  #Och vi levde lyckliga i alla våra dagar!

  def get_connector(side)                           ## returns the out-value as a connector.
...
=begin
      Errors:
      "9*c=f-2" --> Parse error. expected: '*, /', found '-'
      "9*c=f+2" --> Parse error. expected: '*, /', found '+'
