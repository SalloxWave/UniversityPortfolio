...
def sokoban_load(data, level_no):
	with open(data.levels[level_no], 'r') as level_file:
...
def get_level_no(state=None):
	old_level_no = 0
	if state is not None:
		old_level_no = state['level_no']
	if old_level_no < len(sokoban_data.levels):
		new_level_no = input("\n Select a level (1-{}) or leave empty for level {} | quit = q: ".format(len(sokoban_data.levels), old_level_no+1))
		if new_level_no == 'q':
			os.system('clear')
			sys.exit()
		if new_level_no == '':
		...
		new_level_no = input("\n Select a level (1-{}) | quit = q: ".format(len(sokoban_data.levels)))
		if new_level_no == 'q':
			os.system('clear')
			sys.exit()
		else:
...
while True:
	display(state)
	print(" move = hjkl | restart = r | quit = q\n")
